...
export fn update(game_state_ptr: *anyopaque) void {
    ...
    if (p1_upper_bound <= 0 or p1_lower_bound >= game_state.window_height) game_state.p1_dir *= -1;

    if (p2_upper_bound <= 0 or p2_lower_bound >= game_state.window_height) game_state.p2_dir *= -1;

    if (game_state.p1_dir >= 0) game_state.p1_y += 5;
    if (game_state.p1_dir < 0) game_state.p1_y -= 5;

    if (game_state.p2_dir >= 0) game_state.p2_y += 5;
    if (game_state.p2_dir < 0) game_state.p2_y -= 5;
